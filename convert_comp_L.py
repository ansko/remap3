...
def add_mmt(**kwargs):
    ''' Add (construct, NOT convert) MMT into the mesoscopic data.
    '''
    ##########
    ...
    # Charging beads
    assert(len(meso_data.atoms) >= 108)
    idcs_charged = set()
...
def convert_comp_L(in_fname, out_fname):
    ''' Main function, performing complete conversion procedure.
        **********
        Parameters:
        MD                          DPD
        lx = 93.49045391258107      lx' = lx / r_c
        ly = 80.20693670840024      ly' = ly / r_c
        lz = 62.676470142867544     lz' = (lz - mmt_thickness) / r_c
                                    rho = 3
        N = 48420                   N' = 90*20 + 108*(1+tail)

        r_c = (rho * lx*ly*(lz-mmt_th) / N')**(1/3)
        r_c = (3 * 93.49045391258107*80.20693670840024*55 / 2124) ** (1/3)
        r_c = 8.3
    '''
    #########
...
if __name__ == '__main__':
    in_fname = 'md_comp_L.data'
    out_fname = 'meso_comp_L.data'
    convert_comp_L(in_fname, out_fname)
